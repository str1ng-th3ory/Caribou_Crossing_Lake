...
# 03_crossing_event.R
# Purpose:
#   Identify known crossing events and extract event-level movement
#   and albedo/APR attributes for confirmed on-lake crossing points.
#
# Notes:
...
#   - on_lake.csv or equivalent object from Step 2 of the previous script
# ----------------------------------------
...
# option1: load from a Google Sheet (not run by default)
# url = "link"
...
for (i in 1:length(initial_list$Year)) {
  ...
  reference_list = list()
  reference_offsets = c(-1, -2, -3, 1, 2, 3)
  ...
  for (offset in reference_offsets) {
    ...
    if (k > 0 && k < nrow(whole_line)) {
      ...
      displacement = geosphere::distm
      (nearest_location_location, 
        c(nearest_location_next$Lon, nearest_location_next$Lat), 
